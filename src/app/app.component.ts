  ...
  animations: [
    trigger('fade', [
      transition(':enter', [
        ...
        query('.bookmark', animate(300, style({ opacity: 1 }))),
        query('h1, .name', animate(500, style({ opacity: 1 }))),
        query('h2', animate(500, style({ opacity: 1 }))),
        query('#info-text, #skill-text, .description, #contact-text, #contact-list', animate(500, style({ opacity: 1 }))),
        query('#header-container, #skills-frame, .preview, .project-bottom', animate(800, style({ opacity: 1 }))),
      ])
    ])
  ]
})
